...
export type PseuplexMetadataProviderParams = {
	...
	// Options when transforming metadata into plex metadata
...
export type PseuplexMetadataChildrenProviderParams = {
	...
	// Whether to include the "unavailable" status on the metadata if it's not available
...
export type PseuplexMetadataProviderOptions = {
	...
	relatedHubsProviders?: PseuplexHubProvider[];
	plexIdToInfoCache?: PlexIdToInfoCache;
	logger?: Logger;
...
export abstract class PseuplexMetadataProviderBase<TMetadataItem> implements PseuplexMetadataProvider {
	...
	readonly relatedHubsProviders?: PseuplexHubProvider[];
	...
	async getPlexGUIDForID(id: PseuplexPartialMetadataIDString, context: PseuplexRequestContext): Promise<string | null | undefined> {
		...
		return await this.idToPlexGuidCache.set(id, (async () => {
			...
			const matchingMetadata = await findMatchingPlexMetadataItem(this.plexMetadataClient, {
				...matchParams,
				includeFields: ['guid',...PlexIdToInfoCache.fields],
				includeElements: PlexIdToInfoCache.elements,
			});
			...
			if(this.plexIdToInfoCache) {
				this.plexIdToInfoCache.cacheMetadataItem(matchingMetadata);
			}
			return plexGuid;
		})());
	}
	async attachPlexDataIfAble(metadataId: PseuplexPartialMetadataIDString, metadataItem: plexTypes.PlexMetadataItem, context: PseuplexRequestContext): Promise<plexTypes.PlexMetadataItem> {
		try {
			...
			// parse plex guid
			const guidParts = parsePlexMetadataGuidOrThrow(plexGuid);
			...
			// attach additional plex metadata if able
			const plexInfo = await this.plexIdToInfoCache?.getOrFetch(guidParts.id);
			if(!plexInfo) {
				// couldn't get plex item info from guid
				return metadataItem;
			}
			metadataItem.slug = plexInfo.slug;
			if(plexInfo.index != null) {
				metadataItem.index = plexInfo.index;
			}
			if(plexInfo.parentSlug) {
				metadataItem.parentSlug = plexInfo.parentSlug;
			}
			if(plexInfo.parentRatingKey) {
				metadataItem.parentRatingKey = plexInfo.parentRatingKey;
			}
			if(plexInfo.parentIndex != null) {
				metadataItem.parentIndex = plexInfo.parentIndex;
			}
			if(plexInfo.grandparentSlug) {
				metadataItem.grandparentSlug = plexInfo.grandparentSlug;
			}
			if(plexInfo.grandparentRatingKey) {
				metadataItem.grandparentRatingKey = plexInfo.grandparentRatingKey;
			}
	...
	async getIDForPlexGUID(plexGuid: string, options: {
		...
		if(!metadataItem) {
			...
			this.plexIdToInfoCache?.cacheMetadataItemForPlexId(guidParts.id, metadataTask);
	...
	async get(ids: PseuplexPartialMetadataIDString[], options: PseuplexMetadataProviderParams): Promise<PseuplexMetadataPage> {
		...
		const transformOpts: PseuplexMetadataTransformOptions = options.metadataTransformOptions;
		...
		for(const id of ids) {
			...
			if(plexGuid !== null) {
				...
				const metadataTask = (async () => {
					...
					// cache metadata info
					if(this.plexIdToInfoCache && detailedMetadataItem) {
						this.plexIdToInfoCache.cacheMetadataItem(detailedMetadataItem);
					}
				...
				if(guid) {
					if(!plexMetadataMap[guid]) {
						...
						if(matchMetadata?.guid && !plexMetadataMap[matchMetadata.guid]) {
							plexMetadataMap[matchMetadata.guid] = extPlexTransform.transformExternalPlexMetadata(matchMetadata, this.plexMetadataClient.serverURL, context, transformOpts);
						}
					}
				}
			}
			...
			if(remainingGuids.length > 0) {
				...
				if(this.plexIdToInfoCache) {
					...
					for(const guid of remainingGuids) {
						...
						this.plexIdToInfoCache.cacheMetadataItemForPlexId(guidParts.id, guidMapTask.then((guidMap) => guidMap[guid]));
					}
				}
				...
				if(metadatas) {
					...
					for(const metadata of metadatas) {
						if(metadata.guid) {
							plexMetadataMap[metadata.guid] = extPlexTransform.transformExternalPlexMetadata(metadata, this.plexMetadataClient.serverURL, context, transformOpts);
						}
					}
				}
			}
		...
		const metadatas: PseuplexMetadataItem[] = (await Promise.all(ids.map(async (id) => {
			...
			if(metadataItem) {
				...
				if(options.transformMatchKeys || !metadataItem.Pseuplex.isOnServer) {
					...
					const idParts = parsePartialPseuplexMetadataID(id);
					const fullMetadataId = stringifyPseuplexMetadataID({
						...idParts,
						source: this.sourceSlug,
						isURL: false
					});
					// transform keys back to the original key used to fetch this item
					metadataItem.key = stringifyPseuplexMetadataKeyFromIDString(fullMetadataId);
